...
const AUDIENCE_KEYS = ["policymaker", "ngo", "educator", "journalist", "researcher"] as const
...
const AUDIENCE_ICONS = [
  ...
  // ngo
  <svg key="ngo" className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
  </svg>,
  // educator
  <svg key="edu" className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
  </svg>,
  // journalist
  <svg key="jour" className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" />
  </svg>,
  // researcher
  <svg key="res" className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function ContactSection() {
  ...
  const [selectedAudience, setSelectedAudience] = useState<AudienceKey | "other" | "">("")
  const [status, setStatus] = useState<"idle" | "sending" | "success" | "error">("idle")

  const audiences = AUDIENCE_KEYS.map((key, i) => ({
    id: key,
  ...
  const selectedLabel = selectedAudience
    ? audiences.find((a) => a.id === selectedAudience)?.label ?? t("audiences.other")
    : ""
  ...
  return (
    <div className="relative w-full bg-gradient-to-br from-slate-900 via-teal-950 to-blue-950 overflow-hidden">
      ...
      <div className="relative z-10 px-4 py-20 sm:py-28 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-16 items-start">
          {audiences.map((audience, i) => (
            <motion.button
              ...
              viewport={{ once: true }}
              onClick={() => {
                setSelectedAudience(audience.id as AudienceKey)
        ...
        >
          <h3 className="text-xl font-semibold text-white mb-6 text-center">{t("formTitle")}</h3>
          ...
          {status === "success" ? (
            <div className="text-center py-8">
              ...
              <p className="text-white font-semibold text-lg mb-1">{t("successTitle")}</p>
              <p className="text-blue-200 text-sm">{t("successSubtitle")}</p>
              <button
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <input
                ...
                name="_subject"
                value={
                  selectedAudience && selectedAudience !== "other"
                    ? t("subjectWithAudience").replace("{audience}", selectedLabel)
                    : t("subjectDefault")
                }
              />
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setSelectedAudience(e.target.value as AudienceKey | "other" | "")}
                  required
                ...
                >
                  <option value="" disabled>{t("selectBackground")}</option>
                  {audiences.map((a) => (
